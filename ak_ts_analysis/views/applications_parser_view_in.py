from ak_ts_analysis.views.applications_parser_view import make_applications_data
...
def get_applications_data(base_name):
    res_table_base = make_applications_data(base_name)
    ...
    compare_licences = match_phrases(app_applications, app_licences)
    ...
    compare_architecture = match_phrases(app_applications, app_architecture)
    ...
    for slide in res_table_base:
        for i in range(len(res_table_base[slide])):
            ...
            if len(arr_architecture) != 0:
                res_table_base[slide][i].append(str_architecture)
                res_table_base[slide][i].append(round(score, 2))
            else:
                res_table_base[slide][i].append('error')
                res_table_base[slide][i].append(0)
